 ...
 * @param availability
 */
export function itemFound(displayName: string, email: string): string {
    return getRandomResponse([
        `I found ${displayName} with email ${email}.`,
        `It looks like ${displayName} has email ${email} list.`,
    ]);
}
...
export function noListsFound(): string {
    return getRandomResponse([
        `You don't have any person with the name yet.`,
        `I dont find any person with name`,
    ]);
}
